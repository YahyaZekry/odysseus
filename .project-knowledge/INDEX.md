...
> Current goal: First upstream contributions are open — PR #6108 + issue #6109 (contrast fix), all checks green, and a follow-up on the stalled RSS proposal #5688. Contribution branches now live in `YahyaZekry/odysseus-upstream`, since a PR can't cross fork networks from the main repo. Next up: a new **retrieval track** in [[roadmap]] (hybrid search + RRF, reranking/cited RAG, watch-folder indexing) drafted from a SurfSense survey — ideas only, licences checked. `upstream-sync.yml` still needs its PAT fixed. Older goal context below.
