...
> Current goal: **`upstream-sync.yml` is failing nightly (since 2026-08-12, 53 commits queued)** — `SYNC_PR_TOKEN` can't push `.github/workflows/ci.yml` without workflow permission; needs a maintainer decision (see [[roadmap]] Active TODOs). Also fixed: search-provider API keys un-enterable (endpoint regression affecting all 8 key-requiring providers) and the Gotham scene not refreshing in Firefox (service-worker image caching). Uncommitted work in the tree. Older goal context below.
