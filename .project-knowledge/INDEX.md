...
> Current goal: UI-correctness pass now committed and pushed — a WCAG-computed `--on-accent` token fixing 51 hardcoded white-on-accent sites app-wide, the Email settings page finally closable (gear toggle + Escape handled in `ui.js`, where it's actually intercepted), and two account-chip bugs (unread styling overriding `.active`; a dead click zone on the chip's right edge). All verified live in the browser. PR #3 (upstream-sync) still open with green CI. See [[roadmap]] and [[history]]. Older goal context below.
