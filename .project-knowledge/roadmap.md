...
2026-08-16: **`upstream-sync.yml` is broken and has been since 2026-08-12** — 5 consecutive nightly failures, no PR opened, **53 upstream commits now queued**. Blocked on `SYNC_PR_TOKEN` lacking workflow permission: the batch touches `.github/workflows/ci.yml`, and GitHub rejects any PAT push that writes under `.github/workflows/` without it. Needs a maintainer decision — see Active TODOs. Also this session: fixed search-provider API keys being un-enterable (endpoint regression, all 8 key-requiring providers affected) and the Gotham scene not refreshing in Firefox (service-worker image caching). Full detail in [[history]].
...
~~**PR #3** (upstream-sync, 8 commits) is open…~~ — **merged 2026-08-13** (`083d26f`); local `dev` fast-forwarded and in sync. No open PRs right now.
...
- [ ] **Unblock `upstream-sync.yml`** (53 commits queued, failing nightly since 2026-08-12). Pick one: **(a)** add workflow permission to the `SYNC_PR_TOKEN` fine-grained PAT ("Workflows: Read and write") — smallest change, keeps the current design; **(b)** split the credentials so the *push* uses the default `GITHUB_TOKEN` (which can write workflow files given `contents: write`) and only PR creation uses the PAT — the PAT was only ever needed for the PR step, per the 2026-08-05 finding; **(c)** exclude `.github/workflows/**` from the sync — not recommended, silently drops upstream CI changes. *(added: 2026-08-16)*
