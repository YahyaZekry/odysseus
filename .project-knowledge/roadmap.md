...
- [ ] **`upstream-sync.yml` needs a `SYNC_PR_TOKEN` repo secret before it can actually open PRs** — the workflow (daily cron + manual dispatch, diffs against `odysseus-dev/odysseus:dev`) works end-to-end except the final `gh pr create` step, which the default `GITHUB_TOKEN` can't do on this repo even with "Allow GitHub Actions to create and approve pull requests" enabled (confirmed via 3 live failing runs). User needs to create a fine-grained PAT (repo-scoped to `YahyaZekry/odysseus`, Contents + Pull requests read/write) and add it as a repo secret named `SYNC_PR_TOKEN`. Until then, the workflow still pushes the merged `upstream-sync` branch successfully — just no PR gets opened automatically. *(added 2026-08-05)*
