...
- [x] **Nested groups** — added `parent_id`, tree UI with collapse/expand, group CRUD supports nesting *(fixed: 2026-06-13)*
- [x] **OPML import preserves group hierarchy** — `parse_opml` returns `group_path`, `import_opml` creates `FeedGroup` records along the path *(fixed: 2026-06-13)*
- [x] **OPML import prevents duplicates** — matches against both raw and resolved URL via `or_()` *(fixed: 2026-06-13)*
- [x] **OPML file upload** — `<input type="file">` in OPML modal *(fixed: 2026-06-13)*
- [x] **Multi-select feeds** — checkboxes + batch action bar with move to group / delete *(fixed: 2026-06-13)*
- [x] **Collapse/expand UX** — single-click toggles, double-click selects, toggle enlarged 12→18px *(fixed: 2026-06-13, awaits testing)*
- [x] **Select mode for checkboxes** — "☐" toggle hides checkboxes by default, feed items toggle batch in select mode *(fixed: 2026-06-13, awaits testing)*
- [x] **Group AI summary** — backend endpoint + frontend button for bullet-point digest of group's articles *(fixed: 2026-06-13, awaits testing)*
...
- **Kimi is registered in all four provider lists** — `static/js/slashCommands.js` (setup shortcuts), `static/js/providers.js` (`_ENDPOINT_LABELS`), `src/llm_core.py` (`_provider_label`), **and** the `#adm-epProvider` `<option>` in `static/index.html`. Missing the 4th list caused the Settings dropdown not to show it. *(fixed 2026-06-19)*
